...
func (r *authRepository) DeleteOnePlayerCredential(pctx context.Context, crendentialId string) (int64, error) {
	ctx, cancel := context.WithTimeout(pctx, 10*time.Second)
	defer cancel()
	...
	result, err := col.DeleteOne(ctx, bson.M{"_id": utils.ConvertToObjectId(crendentialId)})
	if err != nil {
